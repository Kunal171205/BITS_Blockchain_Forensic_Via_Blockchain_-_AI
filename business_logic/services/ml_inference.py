...
def get_model_status():
    ...
    if _pipeline is None:
        ...
        try:
            ...
            return {"model_loaded": False, "error": "Pipeline initialization failed"}
            
    return {
        "model_loaded": _pipeline.config.seg_model is not None,
        "reader_loaded": _pipeline.config.reader is not None,
        "model_path": _pipeline.config.model_save_path,
        "model_exists": os.path.exists(_pipeline.config.model_save_path)
    }
